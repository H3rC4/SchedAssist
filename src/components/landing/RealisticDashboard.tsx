...
const MiniStatCard = ({ name, value, icon: Icon }: { name: string, value: string, icon: LucideIcon }) => (
    <div className="bg-white/[0.04] backdrop-blur-md rounded-2xl p-4 border border-white/[0.06] flex items-center justify-between group hover:bg-white/[0.06] transition-all">
        ...
        <div className="h-10 w-10 rounded-xl bg-white/[0.06] flex items-center justify-center border border-white/[0.06] group-hover:scale-110 group-hover:bg-accent-500 transition-all">
            <Icon className="h-5 w-5 text-white/50 group-hover:text-[#090a0d]" />
        </div>
    </div>
)

const AppointmentItem = ({ name, time, status, service }: { name: string, time: string, status: string, service: string }) => (
    <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06] hover:border-accent-500/30 transition-all group">
        <div className="h-12 w-12 rounded-xl bg-white/[0.06] flex flex-col items-center justify-center border border-white/[0.06] group-hover:bg-accent-500 transition-all">
            <span className="text-[8px] font-black text-white/40 group-hover:text-[#090a0d] uppercase leading-none mb-0.5">Hoy</span>
            <span className="text-sm font-black text-white group-hover:text-[#090a0d] leading-none">{time}</span>
        </div>
        <div className="flex-1">
            ...
            <p className="text-[10px] font-bold text-white/30">Confirmado vía WhatsApp</p>
        </div>
        <div className={`px-2 py-1 rounded-full text-[7px] font-black uppercase tracking-widest border ${
            status === 'confirmed' ? 'bg-accent-500/10 text-accent-400 border-accent-500/20' : 'bg-white/[0.04] text-white/40 border-white/[0.06]'
        }`}>
            {status}
        </div>
    </div>
...
export function RealisticDashboard() {
    return (
        ...
        >
            <div className="absolute top-0 right-0 h-64 w-64 bg-accent-500/[0.06] blur-[100px] rounded-full -mr-32 -mt-32" />
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-8 space-y-6">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] mt-1">Sincronizado con WhatsApp</p>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <MiniStatCard name="Citas Hoy" value="24" icon={Calendar} />
                        <MiniStatCard name="Pacientes" value="1.2k" icon={Users} />
                    </div>

                    <div className="space-y-3">
                        <p className="text-[10px] font-black text-white/30 uppercase tracking-widest px-2">Próximos Turnos</p>
                ...
                <div className="lg:col-span-4 flex flex-col gap-6">
                    <div className="flex-1 bg-white/[0.03] border border-white/[0.06] rounded-3xl p-6 flex flex-col items-center justify-center text-center">
                        <div className="h-16 w-16 rounded-2xl bg-accent-500/10 flex items-center justify-center mb-4">
                        ...
                        <div className="relative h-24 w-24">
                            <svg className="h-full w-full -rotate-90" viewBox="0 0 36 36">
                                ...
                                <circle className="text-accent-500" stroke="currentColor" strokeWidth="4" fill="transparent" strokeDasharray="100, 100" strokeLinecap="round" r="16" cx="18" cy="18" style={{ strokeDashoffset: 100 - 65 }} />
                            </svg>
                    ...
                    <div className="p-6 bg-accent-500 rounded-3xl text-[#090a0d]">
