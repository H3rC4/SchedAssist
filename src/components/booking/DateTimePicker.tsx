    ...
    >
      <div className="flex items-center justify-between mb-8">
        ...
        >
          ...
          {t.change_professional}
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 flex-1">
        {/* Date Selector - New Calendar */}
        <div className="space-y-4">
          <DateGrid
            selectedDate={selectedDate}
            onSelectDate={(date) => { onSelectDate(date); onSelectSlot('') }}
            primaryColor={primaryColor}
            secondaryColor={secondaryColor}
            locale={locales[lang]}
            dayNames={dayNameSets[lang]}
            maxDate={undefined}
          />
        </div>
        ...
        <div className="space-y-4 flex flex-col">
          <p className="text-[10px] font-black text-[#191c1e]/30 uppercase tracking-[0.4em] ml-1">
            {t.available_slots}
          </p>
          <div className="grid grid-cols-3 gap-2 overflow-y-auto max-h-[280px] pr-2 custom-scrollbar">
            {isBlocked ? (
              <div className="col-span-3 py-10 text-center">
                <p className="text-sm font-black text-red-600 uppercase tracking-widest mb-1">
                  {t.unavailable}
                </p>
                {blockReason && (
                  <p className="text-xs text-red-400/60 font-semibold">{blockReason}</p>
                )}
              </div>
