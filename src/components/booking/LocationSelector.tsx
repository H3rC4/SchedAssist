...
export function LocationSelector({ locations, primaryColor, onSelect, t }: LocationSelectorProps) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="relative z-10">
      <div className="mb-8">
        <h2 className="text-2xl font-black text-[#191c1e] mb-2 tracking-tighter uppercase">{t.select_location || 'Selecciona la Sede'}</h2>
        <p className="text-[10px] font-black text-[#191c1e]/40 uppercase tracking-[0.4em]">{t.select_location_desc || '¿A cuál sucursal deseas asistir?'}</p>
      </div>
