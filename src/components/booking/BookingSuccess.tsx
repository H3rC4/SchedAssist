...
export function BookingSuccess({ selectedDate, selectedSlot, primaryColor, secondaryColor, t, lang = 'es', onNewAppointment }: BookingSuccessProps) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2rem] p-10 md:p-12 text-center shadow-card border border-on-surface/5">
      ...
      <p className="text-sm font-bold text-[#191c1e]/40 mb-8 max-w-md mx-auto leading-relaxed">
        {t.confirmation_message || 'Hemos recibido'},
        <strong className="text-[#191c1e]">{format(selectedDate, "d 'de' MMMM", { locale: dateLocales[lang] || es })}</strong>
        {t.at || 'a las'} <strong className="text-[#191c1e]">{selectedSlot}</strong>.
        {t.confirmation_soon || 'Te enviaremos una confirmación pronto.'}
      </p>
      <button onClick={onNewAppointment || (() => window.location.reload())} className="font-black uppercase tracking-[0.2em] text-xs px-10 py-4 rounded-xl transition-all active:scale-95 text-white shadow-lg" style={{ backgroundColor: secondaryColor }}>
